...
const formatQuestion = (result: QuestionQuery, level: JLPTLevel) => {
	...
	const question: Question = {
		...
		date: result.answeredDate,
		chosenAlternative: result.chosenAlternative,
	}
...
export function useQuestions(level: JLPTLevel) {
	...
	answered_questions.chosen_alternative as chosenAlternative,
	...
	const selectAnsweredByDateMany = async (date: string, limit: number = 5): Promise<Question[]> => {
		...
		const result = await selectQuestionMany(whereClause, Order.DATE, limit);
		return result;
	}

	const insertAnswer = async (question: Question, level: JLPTLevel, aswer: number): Promise<boolean> => {
		const query = `INSERT INTO answered_questions (jlpt_level, chosen_alternative, question_id) VALUES (?,?,?)`;
		try {
			const result = await db.runAsync(query,`'${level}'`, aswer, question.id);
			return true;
		} catch (e) {
			return false;
		}
	...
	const filterAnsweredByRight = (questions: Question[]): Question[] => {
		return questions.filter((question) => question.chosenAlternative === question.correctAlternative);
	}
	...
	return {
		selectById, selectByTagName, selectByType, selectByTypeMany, selectAnsweredByDateMany, filterAnsweredByRight, filterAnsweredByWrong, insertAnswer
	};
}